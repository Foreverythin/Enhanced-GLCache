...
static int prepare_inference_data_xgboost(cache_t *cache) {
  ...
  safe_call(XGDMatrixSetUIntInfo(learner->inf_dm, "group", (const unsigned int*)&n_segs, 1));
...
/* calculate the ranking of all segments for eviction */
/* TODO: can sample some segments to improve throughput */
static int prepare_inference_data_lightgbm(cache_t *cache) {
  GLCache_params_t *params = (GLCache_params_t*)cache->eviction_params;
  learner_t *learner = &params->learner;
  ...
  if (params->learner.n_inference > 0) {
    // safe_call(XGDMatrixFree(learner->inf_dm));
    // INFO("11111\n");
    safe_call(LGBM_DatasetFree(learner->inf_dm));
    // INFO("22222\n");
  }
  // safe_call(XGDMatrixCreateFromMat(learner->inference_x, n_segs,
  //                                  learner->n_feature, -2, &learner->inf_dm));
  ...
  LGBM_DatasetCreateFromMat(learner->inference_x, C_API_DTYPE_FLOAT32, n_segs,
                            learner->n_feature, 1, "max_bin=255", nullptr, &learner->inf_dm);
...
void inference_xgboost(cache_t *cache) {
  GLCache_params_t *params = (GLCache_params_t*)cache->eviction_params;
  learner_t *learner = &params->learner;
  // INFO("inference_xgboost\n");
  int n_segs = prepare_inference_data_xgboost(cache);
  // INFO("prepare_inference_data done\n");
...
void inference_lightgbm(cache_t *cache) {
  GLCache_params_t *params = (GLCache_params_t*)cache->eviction_params;
  learner_t *learner = &params->learner;

  int n_segs = prepare_inference_data_lightgbm(cache);
  ...
  // LightGBM预测替换示例
  int64_t out_len = learner->n_inference; // 假设learner->n_inf_samples是需要预测的样本数量
  // 为预测结果分配内存，这里假设是回归问题，每个样本有一个预测值
  pred = (double*)malloc(sizeof(double) * out_len);

// 调用LightGBM的预测函数
// 注意调整参数以匹配您的具体需求，比如数据的类型和布局

LGBM_BoosterPredictForMat(
    learner->booster, 
    (void*)learner->inference_x, C_API_DTYPE_FLOAT32, 
    learner->n_inference, learner->n_feature, 1, C_API_PREDICT_NORMAL,
    0, 0, 
    nullptr, &out_len, pred
);
