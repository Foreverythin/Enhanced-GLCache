...
bool prepare_one_row(cache_t *cache, segment_t *curr_seg, bool is_training_data,
  ...
  //x[6] = (feature_t)curr_seg->num_most_obj;    // Here by Pangyu Li
  // INFO("num_most_obj: %d\n", curr_seg->num_most_obj);
...
static int cmp_train_y(const void *p1, const void *p2) {
  const train_y_t *d1 = (train_y_t*)p1;
  const train_y_t *d2 = (train_y_t*)p2;
...
static void prepare_training_data_per_package(cache_t *cache) {
  ...
  ////////// Here by Pangyu Li, for LightGBM //////////////

  // safe_call(LGBM_DatasetCreateFromMat(learner->train_x, C_API_DTYPE_FLOAT32,
  //                                     learner->n_train_samples, learner->n_feature,
  //                                     1, "max_bin=255", nullptr, &learner->train_dm_lightgbm));

  // safe_call(LGBM_DatasetCreateFromMat(learner->valid_x, C_API_DTYPE_FLOAT32,
  //                                     learner->n_valid_samples, learner->n_feature,
  //                                     1, "max_bin=255", learner->train_dm_lightgbm, &learner->valid_dm_lightgbm));

  // safe_call(LGBM_DatasetSetField(learner->train_dm_lightgbm, "label",
  //                               learner->train_y, learner->n_train_samples,
  //                               C_API_DTYPE_FLOAT32));

  // safe_call(LGBM_DatasetSetField(learner->valid_dm_lightgbm, "label",
  //                               learner->valid_y, learner->n_valid_samples,
  //                               C_API_DTYPE_FLOAT32));
