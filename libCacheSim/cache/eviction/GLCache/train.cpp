...
static void train_xgboost(cache_t *cache) {
  ...
  // debug_print_feature_matrix(learner->train_dm, 30);
  ...
  // safe_call(XGBoosterSetParam(learner->booster, "eta", "0.15"));  // by Pangyu Li
  // safe_call(XGBoosterSetParam(learner->booster, "max_depth", "6"));  // by Pangyu Li
#if OBJECTIVE == REG
  safe_call(
...
static void train_lightgbm(cache_t *cache) {
  GLCache_params_t *params = (GLCache_params_t*)cache->eviction_params;
    learner_t *learner = &params->learner;

    if (learner->n_train != 0) {
        safe_call(LGBM_BoosterFree(learner->booster));
        safe_call(LGBM_DatasetFree(learner->train_dm_lightgbm));
        safe_call(LGBM_DatasetFree(learner->valid_dm_lightgbm));
    }

    prepare_training_data(cache);

    DatasetHandle eval_dmats[2] = {learner->train_dm_lightgbm, learner->valid_dm_lightgbm};
    static const char *eval_names[2] = {"train", "valid"};
    const char *eval_result;
    double train_loss, valid_loss, last_valid_loss = 0;
    int n_stable_iter = 0;
    ...
    safe_call(LGBM_BoosterCreate(learner->train_dm_lightgbm, "objective=regression", &learner->booster));

    safe_call(LGBM_BoosterAddValidData(learner->booster, learner->valid_dm_lightgbm));

    for (int i = 0; i < N_TRAIN_ITER; ++i) {
      int is_finished;
      LGBM_BoosterUpdateOneIter(learner->booster, &is_finished);
      ...
      // 获取训练和验证损失
      LGBM_BoosterGetEval(learner->booster, 0, &out_len, &out_results[0]); // 0为训练集

      train_loss = out_results[0];
      LGBM_BoosterGetEval(learner->booster, 1, &out_len, &out_results[1]); // 1为验证集

      valid_loss = out_results[1];

      if (fabs(last_valid_loss - valid_loss) / valid_loss < 0.005) {
        n_stable_iter += 1;
        if (n_stable_iter > 2) {
          break;
        }
  ...
  LGBM_BoosterGetLeafValue(learner->booster, 0, 1, &learner->value_tmp);
  INFO(
      "%.2lf hour, cache size %.2lf MB, vtime %ld, train/valid %d/%d samples, "
      "%d trees, %.2lf value_tmp"
      "rank intvl %.4lf\n",
      (double)params->curr_rtime / 3600.0,
      (double)cache->cache_size / 1024.0 / 1024.0, (long)params->curr_vtime,
      (int)learner->n_train_samples, (int)learner->n_valid_samples,
      learner->n_trees, learner->value_tmp, params->rank_intvl);
...
void train(cache_t *cache) {
...
#ifdef LOAD_MODEL
  {
    ...
    safe_call(XGDMatrixCreateFromFile(s, 0, &learner->train_dm));
    INFO("Load model %s\n", s);
  }
#else
  train_xgboost(cache);

  // train_lightgbm(cache);
