...
static void train_lightgbm(cache_t *cache) {
  ...
  if (learner->n_train != 0) {
    ...
    safe_call(LGBM_DatasetFree(learner->train_dm));
    safe_call(LGBM_DatasetFree(learner->valid_dm));
  ...
  /////////// Here !!!!! by Pangyu Li
  // Change XGBoost algo to LightGBM algo
  safe_call(LGBM_BoosterCreate(eval_dmats, 1, &learner->booster));
  safe_call(LGBM_BoosterSetParam(learner->booster, "objective", "regression"));
  safe_call(LGBM_BoosterSetParam(learner->booster, "verbosity", "1"));
  safe_call(LGBM_BoosterSetParam(learner->booster, "num_threads", "1"));
...
#elif OBJECTIVE == LTR
  // safe_call(XGBoosterSetParam(learner->booster, "objective", "rank:pairwise"));
  safe_call(LGBM_BoosterSetParam(learner->booster, "objective", "lambdarank"));
#endif

  for (int i = 0; i < N_TRAIN_ITER; ++i) {
    // Update the model performance for each iteration
    // safe_call(XGBoosterUpdateOneIter(learner->booster, i, learner->train_dm));
    safe_call(LGBM_BoosterUpdateOneIter(learner->booster, i));
    if (learner->n_valid_samples < 10) continue;
    // safe_call(XGBoosterEvalOneIter(learner->booster, i, eval_dmats, eval_names,
    //                                2, &eval_result));
    safe_call(LGBM_BoosterEval(learner->booster, i, eval_dmats, 2, eval_names, &eval_result));
#if OBJECTIVE == REG
    char *train_pos = strstr(eval_result, "train-rmse:") + 11;
    char *valid_pos = strstr(eval_result, "valid-rmse") + 11;
    train_loss = strtof(train_pos, NULL);
...
#error
...
#ifndef __APPLE__
  // safe_call(XGBoosterBoostedRounds(learner->booster, &learner->n_trees));
  safe_call(LGBM_BoosterGetEval(learner->booster, 0, &learner->n_trees));
#endif
...
#ifdef DUMP_MODEL
  {
    ...
    snprintf(s, 128, "dump/model_%d.bin", learner->n_train);
    // safe_call(XGBoosterSaveModel(learner->booster, s));
    safe_call(LGBM_BoosterSaveModel(learner->booster, 0, 0, 0, s));
    INFO("dump model %s\n", s);
  }
#endif
}
