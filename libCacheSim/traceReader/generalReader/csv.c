...
static inline void csv_cb1(void *s, size_t len, void *data) {
  ...
  else if (csv_params->curr_field_idx == 4) {    ///////////// Written by Pangyu Li. This is hard coded for the obj_type field.
    req->obj_type = (uint32_t)strtoul((char *)s, &end, 0);
  }
...
void csv_setup_reader(reader_t *const reader) {
  ...
  csv_params->obj_type_field_idx = init_params->obj_type_field;
